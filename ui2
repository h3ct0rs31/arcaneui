...
function Menu.DrawHeader()
    ...
    -- fondo header
    if Susano and Susano.DrawRectFilled then
        Susano.DrawRectFilled(x, y, width, height, bgR + 0.03, bgG + 0.03, bgB + 0.05, 1.0, radius)
    else
        Menu.DrawRect(x, y, width, height, Menu.Colors.BackgroundDark.r + 8, Menu.Colors.BackgroundDark.g + 8, Menu.Colors.BackgroundDark.b + 13, 255)
    end
    ...
    local titleY = y + (height / 2) - (titleSize * scale * 0.55)
    Menu.DrawTextEmphasis(titleX, titleY, titleText, titleSize, acR, acG, acB, 1.0)

    -- subtitulo
    local subText = Menu.CurrentTheme
    local subSize = 11
    local subW = Menu.GetTextWidth(subText, subSize)
    local subX = x + (width / 2) - (subW / 2)
    local subY = titleY + (titleSize * scale * 0.9) + (2 * scale)
    Menu.DrawText(subX, subY, subText, subSize, 1.0, 1.0, 1.0, 0.35)

    -- linea separadora inferior
    local sepH = math.max(1, 1 * scale)
    local sepY = y + height - sepH
    if Susano and Susano.DrawRectFilled then
        Susano.DrawRectFilled(x, sepY, width, sepH, acR, acG, acB, 0.25, 0)
    else
